...
function P_news = ParticleGeneration_SamplingEfficiency_New(sam, Num_particles)
global alpha;
global N_d 
global Num_candidate;

NSAM = size(sam, 1);                                    % # number of the current samples
cand = rand([Num_candidate, N_d]);                      % Generate candidates from a uniform distribution in the "low dimensional" case
...
for ctr = 1 : Num_candidate
   ...
   if proj_d >= d_min     
       ...
       % Discrepancy
       Discrepancy(ntr, 1) = (13/12)^N_d - D1_r + D2_r;
...
Disc_cand = [Discrepancy, cand_remain];

sorted = sortrows(Disc_cand);
...
if isempty(Discrepancy) == 1
    ...
    for i = 1 : Num_particles
        ...
        New_parts = [New_parts; sorted(i,2), sorted(i,3)];
